...
### 6.0 Tổng Quan Danh Mục Dịch Vụ

Hệ thống phân loại dịch vụ thành **2 danh mục chính (ServiceCategory)**:

| Category | Mô Tả | Đặc Điểm |
|----------|-------|----------|
| `STORAGE` | Dịch vụ gửi đồ | Giá cố định theo thời gian, không tính theo cân nặng |
| `LAUNDRY` | Dịch vụ giặt đồ | Giá tính theo cân nặng (kg), có thể có giá min/max |

**Các ServiceType trong mỗi Category:**

| ServiceType | Category | Mô Tả |
|-------------|----------|-------|
| `STANDARD_DROPOFF` | STORAGE | Gửi đồ tiêu chuẩn |
| `OVERNIGHT` | STORAGE | Gửi đồ qua đêm |
| `EXPRESS_2H` | STORAGE | Gửi đồ nhanh 2 giờ |
| `MONTHLY_STUDENT` | STORAGE | Gói tháng cho sinh viên |
| `MONTHLY_SHIPPER` | STORAGE | Gói tháng cho shipper |
| `LAUNDRY` | LAUNDRY | Dịch vụ giặt |
| `ADDITIONAL_FEE` | LAUNDRY | Phụ phí bổ sung |

> 💡 **Use Case cho Mobile App:** Hiển thị 2 tab/button "Gửi đồ" và "Giặt đồ", khi nhấp vào sẽ lọc danh sách dịch vụ con tương ứng.
...
| Param | Type | Mô tả |
|-------|------|-------|
| category | string | Lọc theo danh mục: `STORAGE` hoặc `LAUNDRY` (optional) |
| storeId | long | Lọc theo cửa hàng (optional) |

**Ví dụ:**
```http
GET /api/services?category=STORAGE
GET /api/services?category=LAUNDRY
GET /api/services?storeId=1&category=STORAGE
```
...
```json
{
  ...
  "data": [
    ...
    {
      "id": 3,
      "name": "Gửi đồ tiêu chuẩn",
      "description": "Gửi đồ trong hộp locker tiêu chuẩn",
      "category": "STORAGE",
    ...
    {
      "id": 4,
      "name": "Gói tháng sinh viên",
      "description": "Gói gửi đồ theo tháng cho sinh viên",
      "category": "STORAGE",
...
### 6.2 Lấy Dịch Vụ Theo Danh Mục (Category)
...
**Ví dụ Response (category=STORAGE):**
```json
{
  "success": true,
  "data": [
...
### 6.3 Lấy Dịch Vụ Theo Cửa Hàng

```http
GET /api/services?storeId=1
GET /api/services?storeId=1&category=LAUNDRY
```

---

### 6.4 Lấy Chi Tiết Dịch Vụ
